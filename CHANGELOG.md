...
- 2026-09-14（再追加）：兩個修正。①**修「設計參考相仲係顯示唔到」嘅真正根因**——`route()` 個 `try { return renderXxx(); } catch {...}` 寫法有個經典 async 陷阱：`return` 咗個 promise 之後 try 已經完成執行，之後嗰個 promise 先至 reject 嘅話，呢個 catch 完全接唔到——變成靜靜雞嘅 unhandled rejection，個頁面凍結喺舊內容度，冇任何錯誤提示。之前個 timeout guard 令逾時會 reject 一個清楚嘅 error，但因為呢個 bug，個 error 由頭到尾冇顯示過，睇落就好似「上載到但顯示唔到」。改法：`route()` 入面全部 render call 都加返 `await`，等 catch 真係接得到。②加埋 per-photo／per-floorplan 嘅本地 try/catch——單一張相 sign URL 失敗（例如撞返 session lock）唔會再累到成個「設計＋平面圖」畫面爆晒，淨係嗰張相顯示唔到，其他部分（平面圖、度尺、其他相片）如常。順手將「📐 設計＋平面圖」呢個之前話難搵嘅入口，同「房間問卷」「需求書」整成一個成日都見到嘅三分頁掣（之前淨係細細個 ghost button 散落喺唔同畫面頂部）。冇改 database；本機驗證咗：（a）故意整一個真係冇被 catch 嘅錯誤，確認而家會show返「出錯喇」畫面而唔係靜靜雞凍結；（b）故意整一個 signed-url 逾時，確認個 gallery 卡仍然完整顯示、淨係嗰張相變空白，0 意外 console error。
